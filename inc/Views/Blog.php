...
class Blog extends Index {
	...
	public function view_hooks() {
		parent::view_hooks();
		if ( is_home() ) {
			add_action( 'fh_before_content', array( get_class( $this ), 'blog_title' ), 2 );
			add_action( 'fh_before_content', array( $this, 'blog_filters' ), 4 );

			add_action( 'fh_before_loop', array( get_class( $this ), 'open_container' ), 10 );
			add_action( 'fh_after_loop', array( get_class( $this ), 'close_container' ), 20 );
			add_action( 'fh_after_loop', array( get_class( $this ), 'pagination' ), 30 );

			add_action( 'fh_entry', array( get_class( $this ), 'loop_entry' ), 5 );
		}
	}
